...
def enforce_either_or_logic(apps, schema_editor):
    ...
    """
    SkinFactContentBlock = apps.get_model('core', 'SkinFactContentBlock')
    
    # Use raw SQL to avoid ORM issues with missing fields
    with connection.cursor() as cursor:
        ...
        for row in cursor.fetchall():
            block_id, content, image = row
            # Clear content if image exists
            cursor.execute("""
...
class Migration(migrations.Migration):
    ...
    operations = [
        # Just enforce the either/or data logic
        migrations.RunPython(enforce_either_or_logic, reverse_migrate_blocks),
        # Update field definitions to match the model (state only)
