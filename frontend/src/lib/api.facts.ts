...
const mapDetail = (raw: RawFactTopicDetail): FactTopicDetail => ({
  ...
  contentBlocks: raw.content_blocks.map(mapBlock),
});

export async function fetchPopularTopics(limit = 5): Promise<FactTopicSummary[]> {
  if (shouldUseMock) {
...
export async function fetchTopicsBySection(
  ...
  const res = await fetch(`${base}/facts/topics/section/${section}?${params.toString()}`, {
    cache: "no-store",
  });
  if (!res.ok) {
...
export async function fetchFactTopicDetail(slug: string): Promise<FactTopicDetail | null> {
  if (shouldUseMock) {
  ...
  const base = getApiBase();
  const res = await fetch(`${base}/facts/topics/${slug}`, { cache: "no-store" });
  if (res.status === 404) {
    return null;
  }
  if (!res.ok) {
