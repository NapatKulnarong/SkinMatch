...
function MatchHistoryPanel({ token }: { token: string | null }) {
  ...
  const handleDelete = async (item: QuizHistoryItem, fallbackKey: string) => {
    if (!token) {
      setError("Sign in to delete match history.");
      return;
    }
    ...
    try {
      await deleteQuizHistoryItem(identifier, token);
      setHistory(prev =>
        prev.filter(existing => {
          if (item.profileId && existing.profileId === item.profileId) {
            return false;
          }
          if (item.sessionId && existing.sessionId === item.sessionId) {
            return false;
          }
          return true;
        })
      ...
      setError(err instanceof Error ? err.message : "Failed to delete this match.");
    }
  };
